...
export default function YogaPracticeDetailsPage() {
        ...
        const load = async () => {
            try {
                ...
                const [content, contentReviews, progressList] = await Promise.all([
                    getYogaById(contentId),
                    getYogaReviews(contentId),
                    getYogaProgress(),
                ]);
                ...
                setError(err.message || "Failed to load yoga session");
            } finally {
                setLoading(false);
            }
        };
    ...
    const handlePostReview = async () => {
        ...
        try {
            await createYogaReview(contentId, { rating, comment: reviewText.trim() || undefined });
            const next = await getYogaReviews(contentId);
            setReviews(next);
            setRating(0);
            setReviewText("");
        } catch (err: any) {
                ...
                <section className="grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_320px] gap-6 items-start">
                    <div className="space-y-5">
                        <div className="relative rounded-3xl overflow-hidden border border-slate-200 bg-black/80">
                            <div className="absolute left-4 top-4 z-10">
                                <Link href="/yogaprograms" className="w-10 h-10 rounded-full bg-white/20 backdrop-blur-sm text-white flex items-center justify-center">
                        ...
                        <div>
                            ...
                            <p className="text-slate-500 mt-1">
                                {item.subtitle || "Guided Yoga Session"} • {formatDuration(item.duration_seconds)} • <span className="text-indigo-600 font-medium capitalize">{item.difficulty || "all levels"}</span>
                            </p>
                        </div>
                        ...
                        <section className="space-y-3">
                            <h2 className="text-4xl font-bold">About this Practice</h2>
                            <p className="text-slate-600 leading-relaxed">{item.description || "Practice mind-body movement at your pace and track your progress over time."}</p>
                        </section>
                        ...
                        <section className="space-y-3">
                            ...
                            <article className="bg-white border border-slate-200 rounded-2xl p-4 shadow-sm space-y-3">
                                ...
                                />
                                <button onClick={handlePostReview} className="h-10 px-4 rounded-xl bg-indigo-600 text-white font-semibold">Post Review</button>
                            </article>
                    ...
                    <aside className="space-y-5 lg:sticky lg:top-24">
                        <article className="bg-white border border-slate-200 rounded-2xl p-5">
                            ...
                            <button onClick={() => syncProgress("in_progress")} className="mt-5 w-full h-12 rounded-2xl bg-indigo-600 hover:bg-indigo-700 text-white font-semibold transition">
                                Sync Progress
                            </button>
                        </article>
